...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        "*** YOUR CODE HERE ***"
        ################################################################################################################
        # Create a variable to store the step size (alpha)
        step_size = self.learning_rate

        # Create a variable to store a counter of the number of iterations performed
        counter = 0  # counter is used to access the correct index in grad_tensors

        # ==============================================================================================================
        # For each parameter variable, use gradient descent to compute the updated parameter value
        # ==============================================================================================================
        for param_var in param_vars:

            # Create a variable to store the parameter variable to be updated
            param_var_to_update = param_var

            # Create a variable to store the gradient of the loss
            gradient_of_loss = grad_tensors[counter]

            # Compute the updated parameter value using gradient descent
            updated_parameter_tensor = (param_var - (step_size * gradient_of_loss))

            # Create a tuple to store the parameter variable before updates and the updated parameter value
            update = (param_var_to_update, updated_parameter_tensor)

            # Append the tuple to the list of updates
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        # Create a variable to store the momentum
        momentum = self.momentum

        # Create a variable to store a counter of the number of iterations performed
        counter = 0  # counter is used to access the correct index in vel_vars & grad_tensors

        # ==============================================================================================================
        # For each parameter variable, use gradient descent to compute the updated parameter & velocity values
        # ==============================================================================================================
        for param_var in param_vars:
            ...
            # Create a variable to store the velocity variable to be updated
            velocity_var_to_update = vel_vars[counter]

            # Compute the updated velocity value
            updated_velocity_value = ((momentum * velocity_var_to_update) - (step_size * grad_tensors[counter]))

            # Compute the updated parameter value
            updated_parameter_value = (param_var + updated_velocity_value)

            # Create a tuple to store the velocity variable before updates and the updated velocity value
            velocity_update = (velocity_var_to_update, updated_velocity_value)

            # Create a tuple to store the parameter variable before updates and the updated parameter value
            parameter_update = (param_var_to_update, updated_parameter_value)

            # Append the tuples to the list of updates
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            "*** YOUR CODE HERE ***"
            ############################################################################################################
            # Create a variable for the loss of an iteration through the neural network using all of the training data
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: train_data[0], placeholders[1]: train_data[1]})

            # Use the loss tensor (train_loss) to update the neural network parameters
            session.run(update_ops, feed_dict={placeholders[0]: train_data[0], placeholders[1]: train_data[1]})

            # Create a variable for the loss of an iteration through the neural network using all of the validation data
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        "*** YOUR CODE HERE ***"
        ################################################################################################################
        # Instantiate a generator for the training data
        generator_train = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)

        # Instantiate a generator for the validation data
        generator_val = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        ################################################################################################################
        loss_tensor = self.get_loss_tensor(model.prediction_tensor, target_ph, model.get_param_vars(regularizable=True))
        updates = self.get_updates(loss_tensor, model.get_param_vars(trainable=True))
        update_ops = [tf.assign(old_var, new_var_or_tensor) for (old_var, new_var_or_tensor) in updates]
        ...
        for iter_ in xrange(self.iterations):
            ...
            "*** YOUR CODE HERE ***"
            ############################################################################################################
            # Create variables so that .next() is only called once for each data set during an iteration
            batch_train = generator_train.next()
            batch_val = generator_val.next()

            # Create a variable for the loss of an iteration using only the training data that was used for the updates
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: batch_train[0], placeholders[1]: batch_train[1]})

            # Use the loss tensor (train_loss) to update the neural network parameters
            session.run(update_ops, feed_dict={placeholders[0]: batch_train[0], placeholders[1]: batch_train[1]})

            # Create a variable for the loss of an iteration using the same amount of validation data as the updates
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: batch_val[0], placeholders[1]: batch_val[1]})
            ############################################################################################################
            # util.raiseNotDefined()
            """
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        trainGenerator = MinibatchIndefinitelyGenerator( train_data, self.batch_size, True )
        valGenerator   = MinibatchIndefinitelyGenerator( val_data  , self.batch_size, True )
        for iter_ in range(self.iterations):
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
            # val_loss should be the loss of this iteration using the same amount of data used for the updates, but using the validation data instead
            "*** YOUR CODE HERE ***"
            ############################################################################################################
            # Create variables so that .next() is only called once for each data set during an iteration
            batch_train = trainGenerator.next()
            batch_val = valGenerator.next()
